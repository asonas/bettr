...
- [ ] **Step 1: Write the failing decision-read test**
...
- [ ] **Step 2: Run the read test and verify the expected red result**

Run: `mise exec -- cargo test --lib store::tests::list_decisions_returns_requests_in_creation_order`
...
- [ ] **Step 3: Implement the audited decision read**
...
- [ ] **Step 4: Run the read test and verify green**

Run: `mise exec -- cargo test --lib store::tests::list_decisions_returns_requests_in_creation_order`

Expected: PASS, with no new warnings.

- [ ] **Step 5: Write the failing expected-revision test**
...
- [ ] **Step 6: Run the revision test and verify the expected red result**

Run: `mise exec -- cargo test --lib store::tests::decision_resolution_rejects_a_stale_expected_revision`

Expected: compilation failure because the existing resolution signature has no expected-revision parameter.

- [ ] **Step 7: Implement the optional guard without changing the CLI contract**
...
- [ ] **Step 8: Run focused decision tests**

Run: `mise exec -- cargo test --lib store::tests::list_decisions_returns_requests_in_creation_order store::tests::decision_resolution_rejects_a_stale_expected_revision` and `mise exec -- cargo test --test cli_decisions`

Expected: PASS. Existing CLI decision tests must retain their current behavior because the CLI passes no Web-only guard.

- [ ] **Step 9: Commit the contract-preserving store change**
...
- [ ] **Step 1: Extend the test fixture with an open decision and a blocked Issue**
...
- [ ] **Step 2: Write the failing read-model and POST tests**
...
- [ ] **Step 3: Run the focused Web tests and verify the expected red result**
...
- [ ] **Step 4: Add the Web projection types and wait-context derivation**
...
- [ ] **Step 5: Extend detail and list routes**
...
- [ ] **Step 6: Extend the bounded HTTP request parser**
...
- [ ] **Step 7: Implement the exact resolve route**
...
- [ ] **Step 8: Run the focused Web tests and verify green**
...
- [ ] **Step 9: Commit the Web API change**
...
- Create: `tests/frontend/decision.test.js`
...
- [ ] **Step 1: Write failing frontend tests for list/detail context**
...
- [ ] **Step 2: Run the focused frontend tests and verify the expected red result**

Run: `mise exec -- npm run test:frontend -- tests/frontend/detail.test.js tests/frontend/app-controller.test.js tests/frontend/decision.test.js`

Expected: FAIL because the existing controller does not render Web wait data or decision forms.

- [ ] **Step 3: Add semantic read-model rendering**
...
- [ ] **Step 4: Write failing frontend tests for state-specific fields and submission safety**

In `decision.test.js`, mount the existing shell and mock `fetch` for detail GET, resolve POST, and refreshed detail/status responses. Assert one form per open request, native `required` attributes for `todo`, `blocked`, `done`, and `cancelled`, omission of hidden optional fields, one POST despite two submit events, disabled button text while pending, and a successful GET after the POST.
...
- [ ] **Step 5: Run the submission tests and verify the expected red result**

Run: `mise exec -- npm run test:frontend -- tests/frontend/decision.test.js`

Expected: FAIL because no resolve form or POST handler exists.

- [ ] **Step 6: Implement dynamic validation and guarded submission**
...
- [ ] **Step 7: Add accessibility and state styling**
...
- [ ] **Step 8: Run the focused frontend tests and verify green**
...
```sh
git add src/web/app.js src/web/app.css tests/frontend/detail.test.js tests/frontend/app-controller.test.js tests/frontend/decision.test.js tests/frontend/support.js
...
- [ ] **Step 2: Update the operator-facing documentation**
...
Run: `mise exec -- cargo fmt --check`, `mise exec -- cargo test --lib`, `mise exec -- cargo test --test web_api --test cli_decisions`, and `mise exec -- npm test`.
...
Run: `mise exec -- cargo test` and `mise exec -- cargo clippy --all-targets --all-features -- -D warnings`.
