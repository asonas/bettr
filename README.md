...
For normal local use, bettr protects unrelated SQLite databases from accidental selection by commands that require an initialized database. Before those commands ask SQLite to open a path, bettr verifies that it resolves to a regular file, then reads its header without modifying it and checks the bettr application ID and schema version. After opening the database, bettr checks the identity again on that same connection before enabling connection settings such as WAL. If the identity preflight rejects the path, these commands return exit code 3 with `database_not_initialized`; existing file bytes are not changed and bettr does not create `-wal`, `-shm`, or `-journal` sidecars during the header preflight.
