...
When `sqlite_backup_restore` is enabled, `bettr backup --output PATH --json` writes one SQLite online-backup snapshot and refuses an existing output. `bettr restore --input PATH --output PATH --yes --json` validates SQLite identity, supported schema, integrity, and foreign keys before staging a restore. Existing destinations require both `--replace` and `--yes`; SQLite WAL, SHM, and journal sidecars are rejected at backup input and restore destination boundaries. Restore rebuilds the active database-adjacent `.audit.jsonl` from SQLite audit events; archived JSONL generations are not bundled. Redaction, retention, and doctor remain later scope.
